...
psm_handle1 = _client.get_obj_handle('psm/baselink1')
psm_handle2 = _client.get_obj_handle('psm/baselink2')
...
psm_children_names = psm_handle1.get_children_names() # Get a list of children names belonging to this obj
...
for i in range(0, 4000):
    t += 0.002
    ang = 3.14/4*np.sin(2*3.14*t)
    psm_handle1.set_joint_pos(0, ang)     # roll
    psm_handle1.set_joint_pos(1, ang)     # pitch
    psm_handle1.set_joint_pos(4, 0.1)        # insertion
    psm_handle1.set_joint_pos(5, 3.14/2+ang)     # roll (offset)
    psm_handle1.set_joint_pos(6, ang)     # wrist
    psm_handle1.set_joint_pos(7, ang)        # jaw (red) -
    psm_handle1.set_joint_pos(8, ang)        # jaw (black) +

    psm_handle2.set_joint_pos(0, ang)  # roll
    psm_handle2.set_joint_pos(1, ang)  # pitch
    psm_handle2.set_joint_pos(4, 0.1)  # insertion
    psm_handle2.set_joint_pos(5, 3.14 / 2 + ang)  # roll (offset)
    psm_handle2.set_joint_pos(6, ang)  # wrist
    psm_handle2.set_joint_pos(7, ang)  # jaw (red) -
    psm_handle2.set_joint_pos(8, ang)  # jaw (black) +
